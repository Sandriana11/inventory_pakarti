<x-app-layout>
    ...
    <div class="content">
        <div class="content-heading d-flex justify-content-between align-items-center">
            <span>Ubah Perbaikan</span>
        </div>
        <div class="block block-rounded">
            <div class="block-content">
                <form method="POST" action="{{ route('maintenance.update', $data->id) }}">
                    @csrf
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-6">
                            <select class="form-select  {{ $errors->has('crash_id') ? 'is-invalid' : '' }}" id="field-crash_id" style="width: 100%;" name="crash_id" data-placeholder="Pilih Laporan">
                                ...
                                @foreach ($kerusakan as $p)
                                    <option value="{{ $p->id }}"  {{ (old('crash_id', $data->crash_id) == $p->id) ? 'selected="selected"' : '' }}>{{ $p->nomor }}</option>
                                @endforeach
                            </select>
                          ...
                          <div class="col-sm-6">
                              <select class="form-select  {{ $errors->has('eksekutor_id') ? 'is-invalid' : '' }}" id="field-eksekutor_id" style="width: 100%;" name="eksekutor_id" data-placeholder="Pilih Eksekutor">
                                  ...
                                  @foreach ($eksekutor as $p)
                                      <option value="{{ $p->id }}" {{ (old('eksekutor_id', $data->eksekutor_id) == $p->id) ? 'selected="selected"' : '' }}>{{ $p->name }}</option>
                                  @endforeach
                              </select>
