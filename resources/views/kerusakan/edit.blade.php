<x-app-layout>
    ...
    <div class="content">
        <div class="content-heading d-flex justify-content-between align-items-center">
            <span>Ubah Laporan Kerusakan</span>
        </div>
        <div class="block block-rounded">
            <div class="block-content">
                <form method="POST" action="{{ route('crash.update', $data->id) }}">
                    @csrf
                    <div class="row mb-4">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-6">
                            <select class="form-select  {{ $errors->has('lokasi_id') ? 'is-invalid' : '' }}"
                                ...
                                @foreach ($lokasi as $p)
                                <option value="{{ $p->id }}"
                                    {{ (old('lokasi_id',  $data->lokasi_id) == $p->id) ? 'selected="selected"' : '' }}>{{ $p->nama }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-6">
                            <select class="form-select  {{ $errors->has('kategori_id') ? 'is-invalid' : '' }}" 
                                ...
                                <option value="{{ $data->kategori_id }}" selected="selected">{{ $data->kategori->nama }}</option>
                            </select>
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-sm-6">
                            <select class="form-select  {{ $errors->has('barang_id') ? 'is-invalid' : '' }}" 
                                id="field-barang_id" style="width: 100%;" name="barang_id">
                                <option value="{{ $data->barang_id }}" selected="selected">{{ $data->barang->nomor }} : {{ $data->barang->nama }}</option>
                            </select>
